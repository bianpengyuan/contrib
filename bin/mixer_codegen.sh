...
if [ "$opttemplate" = true ]; then
  ...
  go run "${ROOTDIR}/vendor/istio.io/istio/mixer/tools/mixgen/main.go" api -t "$templateDS" --go_out "$templateHG" --proto_out "$templateHSP" "${TMPL_GEN_MAP[@]}"
  ...
  go run "${ROOTDIR}/vendor/istio.io/istio/mixer/tools/mixgen/main.go" template -d "$templateSDS" -o "$templateYaml" -n "$(basename "$(dirname "${template}")")"
...
if [ "$optadapter" = true ]; then
  ...
  go run "${ROOTDIR}/vendor/istio.io/istio/mixer/tools/mixgen/main.go" adapter -c "$adapteCfdDS" -o "$(dirname "${file}")" "${extraflags_array[@]}"
