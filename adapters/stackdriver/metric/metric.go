...
func (b *builder) Build(ctx context.Context, env adapter.Env) (adapter.Handler, error) {
	...
	for name, i := range cfg.MetricInfo {
		mt := i.MetricType
		if mt == "" {
...
func toTypedVal(val interface{}, i info) *monitoringpb.TypedValue {
	...
	switch t := val.(type) {
	case int64:
		return &monitoringpb.TypedValue{Value: &monitoringpb.TypedValue_Int64Value{Int64Value: t}}
	case float64:
		return &monitoringpb.TypedValue{Value: &monitoringpb.TypedValue_DoubleValue{DoubleValue: t}}
	case string:
		return &monitoringpb.TypedValue{Value: &monitoringpb.TypedValue_StringValue{StringValue: t}}
	case bool:
		return &monitoringpb.TypedValue{Value: &monitoringpb.TypedValue_BoolValue{BoolValue: t}}
	}
